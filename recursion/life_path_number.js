...
function lifePathNumber(dateOfBirth) {
    ...
    let arr = dateOfBirth.split("-").map(a => a.split("").map(b => +b).reduce((acc, curr) => acc + curr, 0));
    
    let result = 0
    while(arr.some(a => a>9)){
       arr = arr.map(a => a>9? a.toString().split("").map(b => +b).reduce((acc, curr)=> acc + curr, 0) : a);
    }
    result = arr.reduce((acc, curr) => acc + curr, 0) 
    while(result>9){
      result = result.toString().split("").map(b => +b).reduce((acc, curr) => acc + curr, 0)
    }
